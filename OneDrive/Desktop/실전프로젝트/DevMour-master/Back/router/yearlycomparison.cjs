...
router.get('/risk-prediction', (req, res) => {
    ...
    conn.connect(err => {
        ...
        conn.query(sql, (err, results) => {
            if (err) {
                console.error('❌ 위험도 예측 데이터 조회 실패:', err);
                return res.status(500).json({ error: '위험도 예측 데이터 조회 실패' });
            }
...
router.get('/citizen-report', (req, res) => {
    ...
    conn.connect(err => {
        ...
        conn.query(sql, (err, results) => {
            if (err) {
                console.error('❌ 시민 제보 데이터 조회 실패:', err);
                return res.status(500).json({ error: '시민 제보 데이터 조회 실패' });
            }
    ...
    conn.connect(err => {
        ...
        conn.query(sql, (err, results) => {
            if (err) {
                ...
                return res.status(500).json({ error: '기상 데이터 조회 실패' });
            }
    ...
    conn.connect(err => {
        ...
        Promise.all([
            new Promise((resolve, reject) => {
                conn.query(riskSql, (err, results) => {
                    if (err) reject(err);
                    else resolve(results);
                });
            }),
            new Promise((resolve, reject) => {
                conn.query(reportSql, (err, results) => {
                    if (err) reject(err);
                    else resolve(results);
                });
            }),
            new Promise((resolve, reject) => {
                conn.query(weatherSql, (err, results) => {
                    if (err) reject(err);
                    else resolve(results);
                });
            })
        ]).then(([riskResults, reportResults, weatherResults]) => {
            ...
            console.error('❌ 통합 비교 데이터 조회 실패:', err);
