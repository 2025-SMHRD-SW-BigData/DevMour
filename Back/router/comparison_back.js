...
router.get('/year-over-year', async (req, res) => {
    try {
        ...
        const existingTables = await executeQuery(tableCheckQuery, [dbConfig.database]);
        ...
        if (tableNames.includes('t_risk_prediction')) {
            try {
                ...
                const riskResults = await executeQuery(riskPredictionQuery, [currentYear, currentMonth, lastYear, currentMonth]);
        ...
        if (tableNames.includes('t_citizen_report')) {
            try {
                ...
                const reportResults = await executeQuery(citizenReportQuery, [currentYear, currentMonth, lastYear, currentMonth]);
