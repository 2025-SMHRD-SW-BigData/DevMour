...
// POST 
router.post('/save_weather', async (req, res) => {
    ...
    try {
        ...
        const [results] = await db.execute(checkSQL, [lat, lon]);
        ...
        const [insertResults] = await db.execute(insertSQL, [new Date(),lat, lon, temperature, precipitationValue, snowfallValue, weather ]);
