...
// CCTV 상세 정보 조회
router.get('/detail/:cctvId', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute(sql, [cctvId]);
        ...
        console.error('❌ CCTV 상세 정보 조회 실패:', error);
        res.status(500).json({ error: 'CCTV 상세 정보 조회 실패' });
    }
});

// CCTV 이름 중복 확인
router.get('/check-duplicate/:cctvName', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute(sql, [cctvName]);
        ...
        console.error('❌ CCTV 중복 확인 실패:', error);
        res.status(500).json({ error: 'CCTV 중복 확인 실패' });
    }
});
